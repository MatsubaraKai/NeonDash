// PSOFireworkParticle.cpp
#include "PSOFireworkParticle.h"

/**
 * @file PSOFireworkParticle.cpp
 * @brief 花火用のパイプラインステートオブジェクトを管理するクラスの実装
 */

 /// <summary>
 /// 描画に関する設定をまとめる関数
 /// </summary>
void PSOFireworkParticle::CreatePipelineStateObject() {
    // DirectXCommonのインスタンスを取得
    DirectXCommon* sDirectXCommon = DirectXCommon::GetInstance();

    // ルートシグネチャの作成
    PSOFireworkParticle::CreateRootSignature();
    // 入力レイアウトの設定
    PSOFireworkParticle::SetInputLayout();
    // ブレンドステートの設定
    PSOFireworkParticle::SetBlendState();
    // ラスタライザーステートの設定
    PSOFireworkParticle::SetRasterizerState();
    // デプスバッファの生成
    PSOFireworkParticle::CreateDepth();

    // シェーダーをコンパイルする
    property.vertexShaderBlob = CompileShader(L"Resources/shader/FireworkParticle.VS.hlsl",
    ...
    property.pixelShaderBlob = CompileShader(L"Resources/shader/FireworkParticle.PS.hlsl",
...
/// ルートシグネチャを作成する関数
/// </summary>
void PSOFireworkParticle::CreateRootSignature() {
    // DirectXCommonのインスタンスを取得
    DirectXCommon* sDirectXCommon = DirectXCommon::GetInstance();
...
/// 入力レイアウトを設定する関数
/// </summary>
void PSOFireworkParticle::SetInputLayout() {
...
/// ブレンドステートを設定する関数
/// </summary>
void PSOFireworkParticle::SetBlendState() {
    // ブレンドステートの設定
    // すべての色要素を書き込む
...
/// ラスタライザーステートを設定する関数
/// </summary>
void PSOFireworkParticle::SetRasterizerState() {
    // 裏面（時計回り）を表示しない
...
/// デプスバッファを生成する関数
/// </summary>
void PSOFireworkParticle::CreateDepth()
{
    // デプス機能を有効化する
...
PSOFireworkParticle* PSOFireworkParticle::GetInstance() {
    static PSOFireworkParticle instance;
    return &instance;
}
